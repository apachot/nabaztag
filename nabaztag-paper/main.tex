...
\hypersetup{
  ...
  urlcolor=blue,
  citecolor=blue,
  pdftitle={Reviving Legacy IoT Devices as Embodied AI Interfaces},
  pdfauthor={TBD}
}
...
\title{Reviving Legacy IoT Devices as Embodied AI Interfaces:\\A Software-Only Approach to Sustainable Human--AI Interaction}
\author{Author Name(s)\\
\small Affiliation(s)\\
\small Contact email(s)}
\date{}
...
\textbf{Keywords:} Embodied AI, Human--AI interaction, Tangible interfaces, Legacy IoT, Green IT, Sustainable HCI, Nabaztag, Expressive agents
...
\begin{enumerate}[leftmargin=1.5em]
  \item We describe a software-only architecture for connecting modern LLM and TTS capabilities to a legacy embodied IoT device.
  \item We formalize structured expressive generation, a generation paradigm in which the model outputs a multimodal action specification rather than plain text alone.
  \item We discuss the design and sustainability implications of turning a legacy device into an embodied AI companion for the domestic environment.
\end{enumerate}
...
\begin{itemize}[leftmargin=1.5em]
  \item a recognizable body form
  \item movable ears
  \item controllable light zones
  \item audio playback capabilities
  \item a strong cultural identity as a playful domestic object
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  \item a Flask portal for user accounts, rabbit management, and AI configuration
  \item a FastAPI backend exposing device actions and state
  \item protocol adapters compatible with Nabaztag-era communication flows
  \item a modern LLM layer for response and performance generation
  \item a TTS layer for speech synthesis
\end{itemize}

Each rabbit can be configured with:

\begin{itemize}[leftmargin=1.5em]
  \item a personality prompt
  \item a selected LLM model
  \item a selected TTS voice
  \item randomized intervention parameters
  \item recent conversational memory
\end{itemize}
...
\begin{figure}[t]
  ...
  \fbox{\parbox{0.9\linewidth}{
    \centering
    Figure placeholder: overall system architecture\\
    (portal, API, protocol layer, LLM, TTS, physical rabbit)
  }}
  \caption{High-level architecture of the software-only Nabaztag reactivation stack.}
  \label{fig:architecture}
\end{figure}

\section{Structured Expressive Generation}

The core conceptual contribution of the project is what we call \emph{structured expressive generation}.
...
\begin{itemize}[leftmargin=1.5em]
  \item a text field to be spoken aloud
  \item an action for the left ear
  \item an action for the right ear
  \item a state for each LED zone, including color or blink pattern
\end{itemize}
...
\section{Personality and Emotional Expressivity}
...
\subsection{Ambient Interventions}
...
\subsection{Family and Shared-Space Presence}
...
\section{Sustainability and Green IT Implications}
